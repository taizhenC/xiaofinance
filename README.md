...
```
pipx install infinance     # or: git clone … && uv sync  (then prefix commands with `uv run`)
infinance setup            # fetch MediaCrawler (pinned), install deps + Chromium, scaffold .env
infinance run              # open http://127.0.0.1:8000 — the first-run wizard takes it from here
```
...
Something broken? `infinance doctor` diagnoses and prints the fix for each problem.
...
uv run ruff check infinance tests      # lint
npm ci --prefix frontend               # frontend toolchain (Vite + TS + Preact)
npm run build --prefix frontend        # build web UI into infinance/webui/
npm run dev --prefix frontend          # dev server on :5173, proxies /api to :8000
```

Architecture notes live in [roadmap/technical_architecture.md](roadmap/technical_architecture.md). The crawler sits behind a `SourceProvider` interface (`infinance/providers/`); the pipeline (`ingest → dedup → mentions → score → analyze`) is pure-ish modules over SQLite with versioned migrations.
